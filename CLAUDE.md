...
**Grid mutability invariant**: The grid is a column-major `array[x][y]` where the outer array is shallow-copied on every render (`setGrid([...gridRef.current])`) but the column arrays are **shared and mutated in place** by `swap/` and `clear-blocks/`. This is intentional: mutation lets multiple operations within one tick update the same grid without triggering intermediate renders, and `gridRef` lets the game loop and key handlers read the latest state without stale closures. Consequences: (1) never deep-copy a column inside a swap/clear utility — the caller's ref must see the mutation; (2) always call `setGrid([...gridRef.current])` after mutating to schedule a render; (3) test grids must be gravity-settled (no block floating above an empty cell in the same column).
