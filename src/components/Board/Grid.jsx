...
import {grid as classes} from "../../skins/orange";
...
const Grid = ({grid, rowStart = 0, showSwiper = true, tick = 0, deleted}) => {
        ...
        for (let y = rowStart; y < ROWS + READY_GRID; y++) {
            ...
            for (let x = 0; x < COLUMNS; x++) {
                ...
                if (grid[x][y] === BLOCKS_TYPES.TYPE_A) {
                    gridColumns += `<div class="${classes.gridItem}"><img src="${paths.greyBlock}"/></div>`;
                } else if (grid[x][y] === BLOCKS_TYPES.TYPE_B) {
                    gridColumns += `<div class="${classes.gridItem}"><img src="${paths.orangeBlock}"/></div>`;
                } else if (grid[x][y] === BLOCKS_TYPES.TYPE_A_SPECIAL) {
                    gridColumns += `<div class="${classes.gridItem}"><img src="${paths.greySpecialBlock}"/></div>`;
                } else if (grid[x][y] === BLOCKS_TYPES.TYPE_B_SPECIAL) {
                    gridColumns += `<div class="${classes.gridItem}"><img src="${paths.orangeSpecialBlock}"/></div>`;
                } else if (grid[x][y] === BLOCKS_TYPES.DELETION_TYPE_A) {
                    className = BLOCK_ASSOCIATION[BLOCKS_TYPES.DELETION_TYPE_A];
                    gridColumns += `<div class="${classes.gridItem} ${className}"></div>`;
                } else if (grid[x][y] === BLOCKS_TYPES.DELETION_TYPE_B) {
                    className = BLOCK_ASSOCIATION[BLOCKS_TYPES.DELETION_TYPE_B];
                    gridColumns += `<div class="${classes.gridItem} ${className}"></div>`;
                } else {
                    gridColumns += `<div class="${classes.gridItem}"></div>`;
                }
            }
            let row = classes[`row${y}`];
    ...
    return (
        <div className={classes.board}>
            <div className={classes.grid} dangerouslySetInnerHTML={{__html: blocks}}/>
